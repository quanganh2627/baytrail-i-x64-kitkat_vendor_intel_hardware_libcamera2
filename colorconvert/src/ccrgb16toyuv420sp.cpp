 ...
 */
/*
 * Portions Copyright 2008, Intel Corporation.
 */
...
OSCL_EXPORT_REF ColorConvertBase* CCRGB16toYUV420sp :: New()
{
    CCRGB16toYUV420sp* self = OSCL_NEW(CCRGB16toYUV420sp, ());
    return OSCL_STATIC_CAST(ColorConvertBase*, self);
}


CCRGB16toYUV420sp::CCRGB16toYUV420sp()
{
}


OSCL_EXPORT_REF CCRGB16toYUV420sp::~CCRGB16toYUV420sp()
{
...
void CCRGB16toYUV420sp::freeRGB2YUVTables()
{
...
int32 CCRGB16toYUV420sp::GetOutputBufferSize(void)
{
...
int32 CCRGB16toYUV420sp::SetMode(int32 nMode)
{
    OSCL_ASSERT(_mInitialized == true);
...
void CCRGB16toYUV420sp:: SetColorkey(int16 colorkey)
{
    OSCL_ASSERT(_mInitialized == true);
...
int32 CCRGB16toYUV420sp::SetYuvFullRange(bool range)
{
    OSCL_ASSERT(_mInitialized == true);
    if (range == true)
    {
...
extern "C"
{
    int32 ccrgb16toyuvsp(uint8 *rgb16, uint8 *yuv[], uint32 *param, uint8 *table[]);
    int32 ccrgb16toyuvsp_wo_colorkey(uint8 *rgb16, uint8 *yuv[], uint32 *param, uint8 *table[]);
}

int32 CCRGB16toYUV420sp::Convert(uint8 *rgb16, uint8 *yuv420)
{
    uint32 param[7];
    uint8 *table[3];
    ...
    {
        return ccrgb16toyuvsp(rgb16, yuv, param, table);
    }
    else
    {
        return ccrgb16toyuvsp_wo_colorkey(rgb16, yuv, param, table);
    }
}
...
//FIXME:	for nv12, no need to div by 2
//          tempU += ((pitch_dst - width_dst)) >> 1;
//          tempV += ((pitch_dst - width_dst)) >> 1;
            tempU += ((pitch_dst - width_dst));
            tempV += ((pitch_dst - width_dst));
        }
...
int32 CCRGB16toYUV420sp::Convert(uint8 *rgb16, uint8 **yuv420)
{
    ...
    if (mUseColorKey == true)
    {
        return ccrgb16toyuvsp(rgb16, yuv420, param, table);
    }
    else
    {
        return ccrgb16toyuvsp_wo_colorkey(rgb16, yuv420, param, table);
    }
}
