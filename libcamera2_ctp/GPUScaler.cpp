...
int GPUScaler::addInputBuffer(buffer_handle_t *pBufHandle, int width, int height, int bpl)
{
    ...
    for (int i = 0; i < (MAX_EGL_IMAGE_HANDLE - 1); i++)
    {
        ...
        glTexParameteri_EC(GL_TEXTURE_EXTERNAL_OES, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
        glTexParameteri_EC(GL_TEXTURE_EXTERNAL_OES, GL_TEXTURE_MAG_FILTER, GL_LINEAR_MIPMAP_LINEAR);
