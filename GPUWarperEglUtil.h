...
#ifndef ANDROID_LIBCAMERA_GPUWARPER_EGL_UTIL_H
#define ANDROID_LIBCAMERA_GPUWARPER_EGL_UTIL_H
...
// Enable error checking for egl/gl functions with "_EC" postfix for GPU_WARPING_DEBUG mode
// Disable otherwise with empty string for error check function call
#ifdef GPU_WARPING_DEBUG
#define checkGlError(x)     gpuw_pzEglUtil_checkGlError(x);
#define checkEglError(x)    gpuw_pzEglUtil_checkEglError(x);
#else
...
#define glGetIntegerv_EC(...)                    glGetIntegerv(__VA_ARGS__);                 checkGlError("glGetIntegerv")
...
#endif // ANDROID_LIBCAMERA_GPUWARPER_EGL_UTIL_H
